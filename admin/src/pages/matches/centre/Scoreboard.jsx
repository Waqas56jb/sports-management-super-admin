...
function Side({ team, align, scorers }) {
  return (
    <div className={cn('flex min-w-0 flex-col items-center gap-2 text-center sm:gap-3')}>
      <Link to={`/admin/teams/${team?.id}`} className="rounded-2xl bg-white/5 p-2 ring-1 ring-white/10 transition-colors hover:bg-white/10 sm:p-3">
        <TeamLogo team={team} size="xl" className="size-14 sm:size-20" />
      </Link>
      <Link to={`/admin/teams/${team?.id}`} className="max-w-full truncate text-sm font-semibold text-white hover:underline sm:text-lg">
        {team?.name}
      </Link>
      {scorers.length > 0 && (
        <ul className={cn('hidden space-y-0.5 text-xs text-white/60 sm:block', align === 'right' ? 'text-center' : 'text-center')}>
          {scorers.map((s) => (
      ...
      <div className="relative px-4 pb-5 pt-4 sm:px-8 sm:pb-8 sm:pt-6">
        <div className="flex flex-wrap items-center justify-between gap-2 text-xs sm:text-sm">
          <span className="truncate font-medium text-white/70">
            {match.competition ? (
              <Link to={`/admin/competitions/${match.competition.id}`} className="hover:text-white hover:underline">
        ...
        <div className="mt-5 grid grid-cols-[1fr_auto_1fr] items-start gap-2 sm:mt-6 sm:gap-8">
          <Side team={match.home_team} scorers={goals.filter((g) => g.team_id === match.home_team_id)} />
          ...
          <Side team={match.away_team} scorers={goals.filter((g) => g.team_id === match.away_team_id)} align="right" />
        </div>
