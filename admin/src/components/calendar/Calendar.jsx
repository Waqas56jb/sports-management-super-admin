...
import { CalendarDays, ChevronLeft, ChevronRight, Dumbbell, Trophy } from 'lucide-react';
...
import { formatDate, formatLongDate, localeOf, toISODate, todayISO } from '@/utils/format';
...
const KIND_DOT = { training: 'bg-brand-500', match: 'bg-sky-500', competition: 'bg-amber-500' };
const KIND_ICON = { training: Dumbbell, match: CalendarDays, competition: Trophy };
...
function EventChip({ event, compact }) {
  ...
  return (
    <Link
      to={event.link}
      className={cn('block truncate rounded-md border px-1.5 py-0.5 text-[11px] font-medium leading-4 transition-opacity hover:opacity-80', KIND_STYLES[event.kind])}
  ...
  return (
    <Link to={event.link} className="flex min-h-14 items-center gap-3 rounded-xl border border-line bg-surface px-3 py-2.5 hover:bg-surface-2">
      ...
      <span className="min-w-0 flex-1">
        <span className="block truncate text-sm font-medium text-ink">{label(event, true)}</span>
...
export default function Calendar({ events = [], loading, view, onViewChange, date, onDateChange, className }) {
  ...
  const legend = (
    <ul className="flex flex-wrap gap-x-4 gap-y-1 text-xs text-ink-2">
      {['training', 'match', 'competition'].map((k) => (
