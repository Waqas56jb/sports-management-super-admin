...
import { ClipboardList, CornerDownLeft, Goal, Search, SearchX, Shield, Shirt, Trophy, X } from 'lucide-react';
...
const GROUPS = [
  ...
  { key: 'coaches', icon: ClipboardList },
  ...
  const flat = useMemo(() => {
    ...
    return GROUPS.flatMap((g) =>
      results[g.key].map((item) => {
        ...
        if (g.key === 'players') return { ...base, to: `/admin/players/${item.id}` };
        if (g.key === 'coaches') return { ...base, to: `/admin/coaches/${item.id}` };
        if (g.key === 'teams') return { ...base, to: `/admin/teams/${item.id}` };
        if (g.key === 'matches') return { ...base, to: `/admin/matches/${item.id}` };
        return { ...base, to: `/admin/competitions/${item.id}` };
      }),
    );
  }, [results]);
    ...
    if (group === 'coaches')
      return (
        <>
          ...
          <span className="min-w-0 flex-1">
            <span className="block truncate text-sm font-medium text-ink">{item.name}</span>
            <span className="block truncate text-xs text-ink-3">{item.team?.name ?? t('coaches.unassigned')}</span>
          </span>
        </>
      );
    if (group === 'teams')
