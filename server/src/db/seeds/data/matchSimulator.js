...
export function pickLineup(rng, squad, formation, featuredId) {
  const shape = FORMATION_SHAPE[formation] ?? FORMATION_SHAPE['4-3-3'];
  const shuffled = rng.shuffle(squad.filter((p) => p.status === 'active'));
  // The demo player is a first-choice pick, so their season has something to show.
  const available = [...shuffled.filter((p) => p.id === featuredId), ...shuffled.filter((p) => p.id !== featuredId)];
  const starting = [];
...
export function simulateMatch({ rng, match, players, strength, upToMinute = 90, idPrefix, featuredId }) {
  ...
  for (const [side, teamId] of Object.entries(sides)) {
    ...
    lineups[side] = pickLineup(rng, squad, formation, featuredId);
  ...
  for (const side of ['home', 'away']) {
    ...
    for (const minute of minutes) {
      ...
      const weighted = pool.flatMap((id) => Array(WEIGHT[byId[id].position] * (id === featuredId ? 3 : 1)).fill(id));
      ...
      if (!penalty && rng.chance(0.72)) {
        const assisters = pool.filter((id) => id !== scorer && byId[id].position !== 'goalkeeper');
        const assister = rng.pick(assisters.flatMap((id) => (id === featuredId ? [id, id, id] : [id])));
