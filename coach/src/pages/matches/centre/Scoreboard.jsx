...
function Side({ team, scorers, linkable }) {
  ...
  const to = linkable ? { to: `/coach/teams/${team?.id}` } : {};
  return (
      ...
      <div className="relative px-4 pb-5 pt-4 sm:px-8 sm:pb-8 sm:pt-6">
        <div className="flex flex-wrap items-center justify-between gap-2 text-xs sm:text-sm">
          <span className="truncate font-medium text-white/70">
            {match.competition ? (
              <Link to={`/coach/competitions/${match.competition.id}`} className="hover:text-white hover:underline">
        ...
        <div className="mt-5 grid grid-cols-[1fr_auto_1fr] items-start gap-2 sm:mt-6 sm:gap-8">
          <Side team={match.home_team} scorers={goals.filter((g) => g.team_id === match.home_team_id)} linkable={match.editable_sides?.includes('home')} />
          ...
          <Side team={match.away_team} scorers={goals.filter((g) => g.team_id === match.away_team_id)} linkable={match.editable_sides?.includes('away')} />
        </div>
