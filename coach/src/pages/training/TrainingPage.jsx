...
import { durationMinutes, formatPercent, formatTime, formatWeekday, todayISO } from '@/utils/format';
...
export default function TrainingPage() {
  ...
  // Deep links: /coach/training?new=1&team=t1 (dashboard and team page shortcuts)
  useEffect(() => {
    if (searchParams.get('new') === '1') {
      const team = searchParams.get('team');
      openCreate(team ? { team_id: team } : {});
  ...
  const attendanceCell = (s) =>
    s.status === 'cancelled' ? (
      ...
      <button type="button" onClick={(e) => { e.stopPropagation(); navigate(`/coach/training/${s.id}#register`); }} className="text-xs font-semibold text-amber-700 hover:underline dark:text-amber-300">
        {t('training.actions.attendance')}
      </button>
    ) : (
      <span className="text-xs text-ink-3">{t('training.notRecorded')}</span>
    );

  const cancelledCls = (s) => s.status === 'cancelled' && 'line-through decoration-ink-3/60 opacity-70';
  ...
  const columns = [
    {
      ...
      render: (s) => (
        <div className={cn(cancelledCls(s))}>
  ...
  return (
    <>
      ...
      <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
        {!isCalendar ? (
          <Tabs
            ...
            tabs={['upcoming', 'completed', 'cancelled', 'all'].map((k) => ({ value: k, label: t(`training.tabs.${k}`) }))}
          />
        ) : (
          <span />
        )}
      ...
      {isCalendar ? (
        <CalendarPanel teamId={params.team} kinds={['training', 'match']} />
      ) : (
        <Card>
          ...
          {list.error ? (
            ...
            <DataTable
              ...
              onRowClick={(s) => navigate(`/coach/training/${s.id}`)}
              empty={
                <EmptyState
                  icon={Dumbbell}
              ...
              mobileCard={(s) => (
                <div className="flex items-start gap-3">
                  <div className="min-w-0 flex-1">
                    ...
                    <p className={cn('mt-1.5 text-sm capitalize text-ink-2', cancelledCls(s))}>
                    ...
                    <div className="mt-2">{attendanceCell(s)}</div>
                  </div>
