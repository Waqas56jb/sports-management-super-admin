...
import { AlertCircle, Clock, Lock, Mail, ShieldCheck, Sparkles } from 'lucide-react';
import WhistleIcon from '@/components/common/WhistleIcon';
...
const DEMO = { email: 'coach@example.com', password: 'Coach@123' };
...
export default function LoginPage() {
  ...
  const form = useForm({
    ...
    onSubmit: async (values) => {
      ...
      navigate(location.state?.from ?? '/coach/dashboard', { replace: true });
    },
  });
    ...
    <AuthLayout>
      <div className="animate-slide-up">
        <span className="mb-4 inline-flex items-center gap-1.5 rounded-full bg-brand-50 px-3 py-1 text-xs font-semibold text-brand-800 ring-1 ring-brand-600/20 dark:bg-brand-500/10 dark:text-brand-200 dark:ring-brand-400/25">
          <WhistleIcon className="size-3.5" aria-hidden="true" />
          {t('auth.badge')}
        </span>
        ...
        <form ref={form.formRef} onSubmit={form.handleSubmit} noValidate className="mt-7 space-y-5">
          ...
          <div className="flex flex-wrap items-center justify-between gap-2">
            ...
            <Link to="/coach/forgot-password" className="inline-flex min-h-11 items-center text-sm font-medium text-brand-700 hover:underline dark:text-brand-300 sm:min-h-0">
              {t('auth.forgot')}
            </Link>
          </div>
