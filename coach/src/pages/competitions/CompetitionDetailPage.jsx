...
export default function CompetitionDetailPage() {
  ...
  return (
    <DetailGuard query={query} entity={t('common.competition')} backTo="/coach/competitions" backLabel={t('competitions.detail.back')}>
      {(c) => {
        const mineIds = c.my_teams.map((tm) => tm.id);
        const involvesMine = (m) => mineIds.includes(m.home_team_id) || mineIds.includes(m.away_team_id);
        const highlight = (m) => cn(involvesMine(m) && 'bg-brand-50/60 dark:bg-brand-500/5');
        return (
          <>
            <BackLink to="/coach/competitions">{t('competitions.detail.back')}</BackLink>
            <ProfileHero
              avatar={
            ...
            >
              <div className="flex flex-wrap items-center gap-3 border-t border-line px-4 py-3 sm:px-6">
                <span className="text-xs font-semibold uppercase tracking-wide text-ink-3">{t('competitions.detail.participation')}</span>
                {c.my_teams.map((tm) => (
                  <TeamChip key={tm.id} team={tm} />
                ))}
                <span className="text-xs text-ink-3">· {t('competitions.myMatches', { count: c.my_matches })}</span>
              </div>
            ...
            <div className="mt-4 grid gap-4 sm:mt-6 sm:gap-6 xl:grid-cols-3">
              <Card className="xl:col-span-2">
                ...
                <div className="mt-3">
                  {c.type === 'league' ? (
                    <StandingsTable rows={c.standings} highlight={mineIds} />
                  ) : (
                    <CardBody className="grid gap-2 pt-0 sm:grid-cols-2">
                      {c.teams.map((tm) => (
                        <div key={tm.id} className={cn('flex items-center gap-2 rounded-xl border border-line p-3', mineIds.includes(tm.id) && 'border-brand-400 bg-brand-50/60 dark:border-brand-500/40 dark:bg-brand-500/5')}>
                          <TeamLogo team={tm} size="md" />
                          <span className="flex-1 truncate font-medium text-ink">{tm.name}</span>
                          {mineIds.includes(tm.id) && <Badge tone="brand">{t('common.mine')}</Badge>}
                        </div>
                      ))}
                    </CardBody>
                  )}
                    ...
                    <ol className="space-y-3">
                      {c.top_scorers.map((s, i) => (
                        <li key={s.player_id} className="flex items-center gap-3">
                          ...
                          <PersonCell name={s.player.name} photo={s.player.photo} sub={s.team?.name} to={s.mine ? `/coach/players/${s.player.id}` : undefined} size="sm" className="flex-1" />
            ...
            <div className="mt-4 grid gap-4 sm:mt-6 sm:gap-6 lg:grid-cols-2">
              <Card>
                <CardHeader title={t('competitions.detail.upcoming')} />
                <div className="px-1.5 pb-2 pt-2 sm:px-2">
                  {c.upcoming_matches.length ? c.upcoming_matches.map((m) => <MatchRow key={m.id} match={m} showCompetition={false} className={highlight(m)} linkable={involvesMine(m)} />) : <EmptyState compact icon={CalendarDays} title={t('competitions.detail.noUpcoming')} />}
                </div>
              </Card>
              <Card>
                ...
                <div className="max-h-[560px] overflow-y-auto px-1.5 pb-2 pt-2 scrollbar-thin sm:px-2">
                  {c.completed_matches.length ? c.completed_matches.map((m) => <MatchRow key={m.id} match={m} showCompetition={false} className={highlight(m)} linkable={involvesMine(m)} />) : <EmptyState compact icon={Trophy} title={t('competitions.detail.noCompleted')} />}
                </div>
              </Card>
            </div>
          </>
