  ...
  return (
    <li className={cn('group flex gap-3 px-4 py-4 sm:gap-4 sm:px-5', !n.is_read && 'bg-brand-50/40 dark:bg-brand-500/[0.04]')}>
      ...
      <div className="min-w-0 flex-1">
        <button type="button" onClick={() => onOpen(n)} className="block w-full text-left" disabled={!n.link && n.is_read}>
...
export default function NotificationsPage() {
  ...
  const { params, set } = useListParams({ status: 'all' });
  ...
  const open = async (n) => {
    ...
    else toast.success(t('notifications.toasts.read'));
  };
      ...
      <Card>
        <div className="flex flex-col gap-3 border-b border-line p-3 sm:flex-row sm:items-center sm:justify-between sm:p-4">
          <Tabs
            ...
            tabs={['all', 'unread', 'read'].map((s) => ({ value: s, label: t(`notifications.status.${s}`), count: s === 'unread' ? unread : undefined }))}
          />
