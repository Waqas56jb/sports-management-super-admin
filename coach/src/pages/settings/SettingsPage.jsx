...
import { Check, Database, Globe, KeyRound, Monitor, Moon, Palette, RotateCcw, ShieldCheck, Sun, UserRound } from 'lucide-react';
...
function AccountSection() {
  ...
  const form = useForm({
    ...
    schema: { name: [required, minLength(3), maxLength(80)], email: [required, email], phone: [required, phone] },
    ...
    <Card>
      ...
      <CardBody>
        <form ref={form.formRef} onSubmit={form.handleSubmit} noValidate className="grid max-w-2xl gap-5 sm:grid-cols-2">
          <Input label={t('settings.account.name')} required autoComplete="name" fieldClassName="sm:col-span-2" {...form.field('name')} />
          ...
          <div className="flex flex-col gap-1.5">
            <span className="text-sm font-medium text-ink">{t('settings.account.role')}</span>
