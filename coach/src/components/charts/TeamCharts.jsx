  ...
  return (
    <ResponsiveContainer width="100%" height="100%">
      <LineChart data={data} margin={{ top: 8, right: 12, bottom: 0, left: -12 }}>
        ...
        />
        {teams.map((team) => (
          <Line className="chart-glow" strokeLinecap="round" {...reveal(teams.indexOf(team))} key={team.id} type="monotone" dataKey={team.id} stroke={teamColor(team)} strokeWidth={2} dot={data.length < 16 ? { r: 3, strokeWidth: 0, fill: teamColor(team) } : false} activeDot={{ r: 5, stroke: 'var(--surface)', strokeWidth: 2 }} connectNulls  />
        ))}
        {overallLabel && <Line className="chart-glow" strokeLinecap="round" {...reveal(teams.length)} type="monotone" dataKey="overall" stroke="var(--ink-3)" strokeWidth={2} strokeDasharray="4 4" dot={false} />}
      </LineChart>
    </ResponsiveContainer>
  );
}
