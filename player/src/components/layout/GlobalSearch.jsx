...
import { Bell, CornerDownLeft, Dumbbell, Goal, Search, SearchX, Shield, Trophy, X } from 'lucide-react';
...
function linkFor(group, item) {
  if (group === 'matches') return `/player/matches/${item.id}`;
  if (group === 'training') return `/player/training/${item.id}`;
  if (group === 'team') return item.kind === 'teammate' ? `/player/team?tab=roster&player=${item.id}` : '/player/team';
  if (group === 'competitions') return `/player/competitions/${item.id}`;
  return item.link || '/player/notifications';
}
...
export default function GlobalSearch({ open, onClose }) {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    searchService
      .global(debounced, { trainingLabel: (type) => t(`trainingTypes.${type}`), notificationText })
  ...
  const flat = useMemo(() => {
    ...
    return GROUPS.flatMap((g) =>
      results[g.key].map((item) => ({ group: g.key, item, to: linkFor(g.key, item) })),
    );
  }, [results]);
  ...
  const renderItem = (entry) => {
    ...
    if (group === 'team' && item.kind === 'teammate')
      return (
        <>
          ...
          <span className="min-w-0 flex-1">
            <span className="block truncate text-sm font-medium text-ink">
              {item.name}
              {item.is_me && <span className="ml-1.5 text-xs font-normal text-brand-700 dark:text-brand-400">({t('matches.you')})</span>}
            </span>
    ...
    if (group === 'notifications') {
      ...
      return (
        <>
          <NotificationIcon type={item.type} className="size-8 rounded-lg" />
          <span className="min-w-0 flex-1">
            <span className="block truncate text-sm font-medium text-ink">{text.title}</span>
            <span className="block truncate text-xs text-ink-3">{text.message}</span>
          </span>
        </>
      );
    }
            ...
            GROUPS.filter((g) => results[g.key].length).map((g) => (
              <div key={g.key} className="mb-2" role="group" aria-label={t(`search.groups.${g.key}`)}>
                ...
                {results[g.key].map(() => {
                  ...
                  return (
                    <button
                      key={`${entry.group}-${entry.item.kind ?? ''}-${entry.item.id}`}
                      id={`gs-${i}`}
                      type="button"
                      role="option"
