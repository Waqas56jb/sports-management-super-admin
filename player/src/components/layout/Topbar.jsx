...
export default function Topbar({ onMenu, onSearch, onLogout, collapsed, onToggleSidebar }) {
  const { t } = useI18n();
  const { user } = useAuth();
  ...
  return (
    <header className="glass safe-top sticky top-0 z-30 border-b border-line/70 no-print">
      <div className="flex h-14 items-center gap-2 px-3 sm:h-16 sm:gap-3 sm:px-6">
        <button type="button" onClick={onMenu} className="grid size-10 place-items-center rounded-xl text-ink-2 hover:bg-surface-3 md:hidden" aria-label={t('nav.openMenu')}>
          <Menu className="size-5" />
        </button>
        <button
          type="button"
          onClick={onToggleSidebar}
          className="hidden size-10 place-items-center rounded-xl text-ink-2 hover:bg-surface-3 md:grid"
        ...
        <div className="flex items-center gap-0.5 sm:gap-1">
          ...
          <Dropdown
            ...
            className="ml-1 flex items-center gap-2 rounded-full p-0.5 hover:ring-2 hover:ring-line xl:pr-3"
            trigger={
              <>
                <Avatar name={user?.name} src={user?.avatar} size="sm" />
                <span className="hidden max-w-36 truncate text-sm font-medium text-ink xl:block">{user?.name}</span>
              </>
            }
            header={
            ...
            items={[
              { label: t('nav.profile'), icon: UserRound, to: '/player/profile' },
              { label: t('nav.settings'), icon: Settings, to: '/player/settings' },
