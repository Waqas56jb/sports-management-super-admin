          ...
          <div key={li} className="flex justify-around gap-1">
            {line.map((p) => (
              <div key={p.id} className="flex w-16 flex-col items-center text-center">
                <span
                  className={cn('grid size-9 place-items-center rounded-full border-2 border-white/90 font-display text-sm font-bold text-white shadow-md tabular', p.is_me && 'ring-4 ring-amber-300')}
                  style={{ background: p.position === 'goalkeeper' ? '#111827' : color }}
                >
                  {p.jersey_number ?? '–'}
                </span>
                <span className={cn('mt-1 max-w-full truncate rounded px-1 text-[11px] font-medium leading-4', p.is_me ? 'bg-amber-300 font-semibold text-amber-950' : 'bg-black/35 text-white')}>
                  {p.is_me ? t('matches.you') : surname(p.name)}
                </span>
              </div>
            ))}
          </div>
