 ...
 */
import { extendLine } from '@/data/statisticsData';
...
/**
 * The player's match-by-match lines (with shots, passes, fouls… from statisticsData.extendLine),
 * for completed and live matches in scope, oldest first.
 */
export function playerMatchHistory(db, playerId, filters = {}) {
  const player = db.players.find((p) => p.id === playerId);
  if (!player) return [];
  const byId = Object.fromEntries(db.players.map((p) => [p.id, p]));
  return scopedMatches(db, filters)
    .filter((m) => !filters.matchType || (filters.matchType === 'friendly' ? !m.competition_id : db.competitions.find((c) => c.id === m.competition_id)?.type === filters.matchType))
    .sort((a, b) => `${a.date}${a.time}`.localeCompare(`${b.date}${b.time}`))
    .map((m) => {
      const line = matchPlayerLines(m, db.matchEvents, byId)[playerId];
      return line ? { match: m, line: extendLine(line, player.position, m.id) } : null;
    })
    .filter(Boolean);
}

/** Totals and averages over a list of extended lines. */
export function summarizeLines(lines) {
  ...
  return {
    ...
    pass_accuracy: passes ? Math.round((completed / passes) * 1000) / 10 : null,
    key_passes: sum('key_passes'),
    fouls: sum('fouls'),
    yellow_cards: sum('yellow_cards'),
    red_cards: sum('red_cards'),
    minutes_per_match: played ? Math.round(sum('minutes') / played) : null,
    average_rating: played ? Math.round((sum('rating') / played) * 10) / 10 : null,
  };
}
